...
class Tolak extends BaseController
{
    ...
    public function form_download()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $response->data = view('sek/ppdb/rekap/tolak/download', $x);
                return json_encode($response);
            }
        } else {
    ...
    public function download()
    {
        ...
        try {
            ...
            $worksheet->fromArray(['NO', 'NAMA PESERTA', 'NISN PESERTA', 'TEMPAT LAHIR PESERTA', 'TANGGAL LAHIR PESERTA', 'JENIS KELAMIN', 'NO PENDAFTARAN', 'VIA JALUR', 'JARAK DOMISILI', 'STATUS PENDAFTARAN', 'WAKTU PENDAFTARAN', 'VERIFIKATOR', 'KETERANGAN PENOLAKAN'], NULL, 'A3');
            ...
            if (count($data) > 0) {
                foreach ($data as $key => $item) {
                    ...
                    $worksheet->getCell('M' . $row)->setValue($item->keterangan_penolakan);
