...
class Pendaftar extends BaseController
{
    ...
    {
        return redirect()->to(base_url('sek/ppdb/pendaftar/data'));
    }
    ...
    public function data()
    {
        ...
        return view('sek/ppdb/pendaftar/index', $data);
    }
    ...
    public function detail()
    {
        ...
        if ($oldData->via_jalur == "PRESTASI") {
            return view('sek/ppdb/pendaftar/detail_pres', $data);
        } else {
            return view('sek/ppdb/pendaftar/detail', $data);
        }
    }
    ...
    public function formTolak()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $response->data = view('sek/ppdb/pendaftar/form_tolak', $x);
                return json_encode($response);
            }
        } else {
    ...
    public function formPerubahan()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('sek/ppdb/pendaftar/form_perubahan', $x);
                return json_encode($response);
            }
        } else {
            exit('Maaf tidak dapat diproses');
            ...
            if (!$this->validate($rules)) {
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    if ($this->_db->affectedRows() > 0) {
                        ...
                        $response = new \stdClass;
                        $response->status = 200;
                        $response->url = base_url('sek/ppdb/pendaftar');
    ...
    public function location()
    {
        if ($this->request->isAJAX()) {
            ...
            $response->lat = $lat;
            $response->long = $long;
            $response->data = view('sek/ppdb/pendaftar/maps', $x);
            return json_encode($response);
        } else {
            exit('Maaf tidak dapat diproses');
        }
