...
class Home extends BaseController
{
    ...
    public function getAllSisaKuota()
    {
        $request = Services::request();
        $datamodel = new KuotasisaModel($request);
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $terverifikasi = $this->_db->table('_tb_pendaftar')
                    ->select("id, kode_pendaftaran, npsn_sekolah_asal, via_jalur, nama_peserta as fullname, CONCAT('xx', MID(nisn_peserta,3,6), 'xx') as nisn, nama_sekolah_asal, count(nisn_peserta) as jumlahDaftar")
