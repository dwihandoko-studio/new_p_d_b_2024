        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $terverifikasi = $this->_db->table('_tb_pendaftar')
                    ->select("id, kode_pendaftaran, via_jalur, nama_peserta as fullname, CONCAT('xx', SUBSTRING_INDEX(nisn_peserta, '-', 4), 'xx') as as nisn, nama_sekolah_asal, count(nisn_peserta) as jumlahDaftar")
                ...
                $belumverifikasi = $this->_db->table('_tb_pendaftar_temp')
                    ->select("id, kode_pendaftaran, via_jalur, nama_peserta as fullname, SUBSTRING_INDEX(nisn_peserta, '-', 4), 'xx') as as nisn, nama_sekolah_asal, count(nisn_peserta) as jumlahDaftar")
                    ->where('tujuan_sekolah_id_1', $id)
                    ->groupBy('nisn_peserta')
                    ->orderBy('created_at', 'asc')
                    ->get()->getResult();
