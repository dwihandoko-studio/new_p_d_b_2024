...
class Proses extends BaseController
{
    ...
    public function verified_otomatis_lolos()
    {
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
        ...
        $data = $this->_db->table('_tb_pendaftar_temp')->where('npsn_sekolah_tujuan', '10801743')->get()->getResultArray();
    ...
    // public function proses_tidak_lulusa_afirmasi_sd()
    // {
    //     $Profilelib = new Profilelib();
    //     $user = $Profilelib->user();
    //     if ($user->status != 200) {
    ...
    //             if (count($afirmasiData) > 0) {
    //                 $lulusLib->prosesTidakLulusAfirmasi($afirmasiData, $user->data->id);
    //             }
    //         }
    //         print_r("SELESAI PROSES KELULUSAN ");
    //     } else {
    ...
    // public function proses_tidak_lulusa_afirmasi_smp()
    // {
    //     $Profilelib = new Profilelib();
    //     $user = $Profilelib->user();
    //     if ($user->status != 200) {
    ...
    public function proseskelulusanzonasisd()
    {
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
        ...
        if (count($dataSekolahs) > 0) {
            ...
            foreach ($dataSekolahs as $key => $id) {
                ...
                $limitZonasi = ((int)$kuota->total - (int)$kuota->jumlah_lolos);
            ...
            print_r("SELESAI PROSES KELULUSAN ZONASI ");
        } else {
            print_r("DATA SEKOLAH TIDAK DITEMUKAN");
        }
    }

    public function proseskelulusanzonasismp()
    {
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
    ...
    public function proses_tidak_lulusa_zonasi_sd()
    {
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
        ...
        if (count($dataSekolahs) > 0) {
            ...
            foreach ($dataSekolahs as $key => $id) {
                ...
                if (count($zonasiData) > 0) {
                    $lulusLib->prosesTidakLulusZonasi($zonasiData, $user->data->id);
                }
            }
            print_r("SELESAI PROSES TIDAK KELULUSAN ZONASI ");
        } else {
            print_r("DATA SEKOLAH TIDAK DITEMUKAN");
        }
    }

    public function proses_tidak_lulusa_zonasi_smp()
    {
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
    ...
    public function proseskelulusanprestasidarisisazonasisd()
    {
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
        ...
        if (count($dataSekolahs) > 0) {
            ...
            foreach ($dataSekolahs as $key => $id) {
                ...
                if (((int)$kuota->total - (int)$kuota->jumlah_lolos) < 1) {
                    print_r("SISA KUOTA ZONASI UNTUK PRESTASI SUDAH PENUH <br> ");
                    continue;
                }

                $limitPrestasiSisa = ((int)$kuota->total - (int)$kuota->jumlah_lolos);
                ...
                $prestasisisaZonasiData = $this->_db->table('_tb_pendaftar a')
                    ->select("a.id as id_pendaftaran, a.user_id, a.via_jalur, a.tujuan_sekolah_id_1, a.status_pendaftaran, a.jarak_domisili, a.created_at")
                    ->where('a.tujuan_sekolah_id_1', $id->tujuan_sekolah_id_1)
                    ->where('a.status_pendaftaran', 1)
                    ->where('a.via_jalur', 'PRESTASI')
                    ...
                    ->orderBy('a.jarak_domisili', 'ASC')
                    ->orderBy('a.created_at', 'ASC')
                    ->limit($limitPrestasiSisa)
                    ->get()->getResult();

                $lulusLib = new Prosesluluslib();

                if (count($prestasisisaZonasiData) > 0) {
                    $lulusLib->prosesLulusPrestasi($prestasisisaZonasiData, $user->data->id);
                }
            }
            print_r("SELESAI PROSES KELULUSAN PRESTASI DARI SISA ZONASI ");
        } else {
            print_r("DATA SEKOLAH TIDAK DITEMUKAN");
        }
    }

    public function proseskelulusanprestasidarisisazonasismp()
    {
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
        ...
        if (count($dataSekolahs) > 0) {
            ...
            foreach ($dataSekolahs as $key => $id) {
                ...
                if (((int)$kuota->total - (int)$kuota->jumlah_lolos) < 1) {
                    print_r("KUOTA PRESTASI DARI SISA ZONASI SUDAH PENUH <br> ");
                    continue;
                }
