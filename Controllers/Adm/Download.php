...
class Download extends BaseController
{
            ...
            if (count($data) > 0) {
                foreach ($data as $key => $item) {
                    $queryZonasi = $this->_db->table('_setting_zonasi_tb a')
                        ->select("a.*, f.kecamatan as kec_name, f.nama, f.bentuk_pendidikan_id, f.bentuk_pendidikan, f.status_sekolah, b.nama as nama_provinsi, c.nama as nama_kabupaten, d.nama as nama_kecamatan, e.nama as nama_kelurahan, (SELECT count(dusun) FROM _setting_zonasi_tb WHERE dusun = a.dusun) as jumlah_dusun, count(dusun) as jumlah")
                        ...
                        ->join('dapo_sekolah f', 'a.sekolah_id = f.sekolah_id')
                        ->where('a.sekolah_id', $item->sekolah_id)
                        ->groupBy('a.kelurahan')
                    ...
                    if (count($dataKuota) > 0) {
                        foreach ($dataKuota as $keyKuota => $itemKuota) {
                            ...
                            $worksheet->getCell('B' . $row)->setValue($itemKuota->kec_name);
                            $worksheet->getCell('C' . $row)->setValue($itemKuota->npsn);
                            $worksheet->getCell('D' . $row)->setValue($itemKuota->nama);
                            $worksheet->getCell('E' . $row)->setValue($itemKuota->bentuk_pendidikan);
                            $worksheet->getCell('F' . $row)->setValue($itemKuota->status_sekolah);
                            ...
                            $worksheet->getCell('J' . $row)->setValue($itemKuota->nama_kelurahan);
    ...
    //         // Menulis nama kolom ke dalam baris pertama worksheet
    //         $worksheet->getCell('A1')->setValue("DATA KESIAPAN PEMETAAN ZONASI PPDB");
    //         $worksheet->getCell('A2')->setValue("KABUPATEN LAMPUNG TENGAH");
    //         $worksheet->getCell('A3')->setValue("TAHUN PELAJARAN 2024/2025");
    //         // $worksheet->getCell('A4')->setValue("");
    ...
    //         $worksheet->getColumnDimension('K')->setWidth(30);
    //         // Mengambil data dari database
    //         $query = $this->_db->table('_setting_zonasi_tb a')
    //             ->select("a.*, f.kecamatan as kec_name, f.nama, f.bentuk_pendidikan_id, f.bentuk_pendidikan, f.status_sekolah, b.nama as nama_provinsi, c.nama as nama_kabupaten, d.nama as nama_kecamatan, e.nama as nama_kelurahan, (SELECT count(dusun) FROM _setting_zonasi_tb WHERE dusun = a.dusun) as jumlah_dusun, count(dusun) as jumlah")
    //             ->join('ref_provinsi b', 'b.id = a.provinsi', 'left')
    //             ->join('ref_kabupaten c', 'c.id = a.kabupaten', 'left')
    //             ->join('ref_kecamatan d', 'd.id = a.kecamatan', 'left')
    //             ->join('ref_kelurahan e', 'e.id = a.kelurahan', 'left')
    //             ->join('dapo_sekolah f', 'a.sekolah_id = f.sekolah_id')
    //             ->groupBy('a.npsn')
    //             ->orderBy('f.bentuk_pendidikan_id', 'DESC')
    //             ->orderBy('f.status_sekolah', 'ASC')
    //             ->orderBy('f.nama', 'ASC')
    //             ->orderBy('a.kecamatan', 'asc')
    //             ->orderBy('a.kelurahan', 'asc')
    //             ->get();
    ...
    //             foreach ($data as $key => $item) {
    //                 $worksheet->getCell('A' . $row)->setValue($key + 1);
    //                 $worksheet->getCell('B' . $row)->setValue($item->kec_name);
    //                 $worksheet->getCell('C' . $row)->setValue($item->npsn);
    //                 $worksheet->getCell('D' . $row)->setValue($item->nama);
    //                 $worksheet->getCell('E' . $row)->setValue($item->bentuk_pendidikan);
    //                 $worksheet->getCell('F' . $row)->setValue($item->status_sekolah);
    //                 $worksheet->getCell('G' . $row)->setValue($item->nama_provinsi);
    //                 $worksheet->getCell('H' . $row)->setValue($item->nama_kabupaten);
    //                 $worksheet->getCell('I' . $row)->setValue($item->nama_kecamatan);
    //                 $worksheet->getCell('J' . $row)->setValue($item->nama_kelurahan);
    //                 $namaDusun = getDusunListName($item->kelurahan, $item->sekolah_id);
    //                 $worksheet->getCell('K' . $row)->setValue($namaDusun);
    ...
    //         // Menuliskan file Excel
    //         // if ($jalur == "all") {
    //         $filename = 'DATA_KESIAPAN_PETA_ZONASI_PPDB.xls';
    //         // } else {
    //         //     $filename = 'DATA_PENDAFTAR_' . strtoupper($jalur) . '.xls';
    //         // }
    //         header('Content-Type: application/vnd-ms-excel');
