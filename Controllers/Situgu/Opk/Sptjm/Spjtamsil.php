...
use App\Models\Situgu\Opk\SptjmspjModel;
use Config\Services;
...
class Spjtamsil extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new SptjmspjModel($request);
        ...
        foreach ($lists as $list) {
            ...
            if ($list->is_locked == 1) {
                ...
                if ($list->lampiran_sptjm == null || $list->lampiran_sptjm == "") {
                    $row[] = '<a class="btn btn-sm btn-primary waves-effect waves-light" target="_blank" href="' . base_url('situgu/opk/sptjm/spjtamsil/download') . '?id=' . $list->kode_verifikasi . '"><i class="bx bxs-cloud-download font-size-16 align-middle me-2"></i> Download</a>&nbsp;&nbsp;'
    ...
    {
        return redirect()->to(base_url('situgu/opk/sptjm/spjtamsil/data'));
    }

    public function data()
    {
        $data['title'] = 'SPTJM VERIFIKASI SPJ TUNJANGAN PROFESI GURU';
        ...
        return view('situgu/opk/sptjm/spjtamsil/index', $data);
    }
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->data = view('situgu/opk/sptjm/spjtamsil/detail', $data);
            return json_encode($response);
        }
    }
    ...
    public function addGetTW()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if (count($tws) > 0) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('situgu/opk/sptjm/spjtamsil/get_tw', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function add()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('situgu/opk/sptjm/spjtamsil/add', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function generatesptjm()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if (!$twActive) {
                $response = new \stdClass;
                $response->status = 400;
                $response->message = "Gagal mengenerate SPTJM verifikasi SPJ Tamsil. TW active tidak ditemukan.";
                return json_encode($response);
            }
            ...
            try {
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->message = "SPTJM Verifikasi SPJ Tamsil Tahun {$twActive->tahun} TW {$twActive->tw} berhasil digenerate.";
                    return json_encode($response);
                } else {
                    $this->_db->transRollback();
                    $response = new \stdClass;
                    $response->status = 400;
                    $response->message = "Gagal Mengenerate SPTJM Verifikasi SPJ Tamsil.";
                    return json_encode($response);
                }
            } catch (\Throwable $th) {
                $this->_db->transRollback();
    ...
    public function download()
    {
        ...
        if (!$twActive) {
            $response = new \stdClass;
            $response->status = 400;
            $response->message = "Gagal mendowload SPTJM verifikasi SPJ Tamsil. TW active tidak ditemukan.";
            return json_encode($response);
        }
    ...
    {
        if (count($ptks) > 0) {
            $file = FCPATH . "upload/template/sptjm-verifikasi-spj-tamsil-new-1.docx";
            ...
            $responseD =  $downloadLib->downloaded($filed, $ptks[0]->kode_verifikasi . ".pdf", "spjtamsil");
    ...
    public function formuploadedit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/opk/sptjm/spjtamsil/upload_edit', $data);
            return json_encode($response);
        }
    }
    ...
    public function formupload()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/opk/sptjm/spjtamsil/upload', $data);
            return json_encode($response);
        }
    }
