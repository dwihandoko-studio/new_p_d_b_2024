...
use App\Models\Situgu\Su\Tpg\Upload\PembenahanspjModel;
use Config\Services;
...
class Pembenahanspj extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new PembenahanspjModel($request);
    ...
    {
        return redirect()->to(base_url('situgu/su/upload/tpg/pembenahanspj/data'));
    }

    public function data()
    {
        $data['title'] = 'DATA MATCHING PEMBENAHAN SPJ';
        ...
        return view('situgu/su/upload/tpg/pembenahanspj/index', $data);
    }
    ...
    public function upload()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->data = view('situgu/su/upload/tpg/pembenahanspj/upload', $data);
            return json_encode($response);
        }
    }
    ...
    public function uploadSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $dir = FCPATH . "upload/matching-pembenahanspj";
            $field_db = 'filename';
            $table_db = 'tb_matching_pembenahanspj';
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                $response->data = view('situgu/su/upload/tpg/pembenahanspj/verifi-upload', $x);
    ...
    public function get_data_json()
    {
        ...
        $datas = json_decode(file_get_contents(FCPATH . "upload/matching-pembenahanspj/$id.json"), true);
        ...
        if (isset($datas['data']) && count($datas['data']) > 0) {
            ...
            foreach ($datas['data'] as $key => $v) {
                ...
                if ($v['data_usulan'] == NULL || $v['data_usulan'] == "") {
                    ...
                    $item['tf_gaji_pokok_1'] = "-";
                    $item['tf_gaji_pokok_2'] = "-";
                    $item['tf_gaji_pokok_3'] = "-";
                    $item['tf_jumlah_uang'] = "-";
                    $item['tf_iuran_bpjs'] = "-";
                    $item['tf_pph21'] = "-";
                    $item['tf_jumlah_diterima'] = "-";
                    ...
                    if ((int)$v['data_usulan']['tf_gaji_pokok_1'] == (int)$v['gaji_pokok_1'] && (int)$v['data_usulan']['tf_gaji_pokok_2'] == (int)$v['gaji_pokok_2'] && (int)$v['data_usulan']['tf_gaji_pokok_3'] == (int)$v['gaji_pokok_3'] && (int)$v['data_usulan']['tf_jumlah_uang'] == (int)$v['jumlah_uang'] && (int)$v['data_usulan']['tf_iuran_bpjs'] == (int)$v['iuran_bpjs'] && (int)$v['data_usulan']['tf_pph21'] == (int)$v['pph21'] && (int)$v['data_usulan']['tf_jumlah_diterima'] == (int)$v['jumlah_diterima']) {
                        ...
                        $item['pph21'] = $v['pph21'];
                        $item['jumlah_diterima'] = $v['jumlah_diterima'];
                        $item['tf_gaji_pokok_1'] = $v['data_usulan']['tf_gaji_pokok_1'];
                        $item['tf_gaji_pokok_2'] = $v['data_usulan']['tf_gaji_pokok_2'];
                        $item['tf_gaji_pokok_3'] = $v['data_usulan']['tf_gaji_pokok_3'];
                        $item['tf_jumlah_uang'] = $v['data_usulan']['tf_jumlah_uang'];
                        $item['tf_iuran_bpjs'] = $v['data_usulan']['tf_iuran_bpjs'];
                        $item['tf_pph21'] = $v['data_usulan']['tf_pph21'];
                        $item['tf_jumlah_diterima'] = $v['data_usulan']['tf_jumlah_diterima'];
                        $item['no_rekening'] = $v['no_rekening'];
                        $item['no_sktp'] = $v['no_sktp'];
                        $item['no_urut'] = $v['no_urut'];
    ...
    public function prosesmatching()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            //     ->where('a.id_tahun_tw', $tw)
            //     ->get()->getRowObject();
            $current = $this->_db->table('_tb_spj_tpg')->where('id', $id_usulan)->get()->getRowObject();
    ...
    public function delete()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('tb_matching_pembenahanspj')
                ->where('id', $id)
                ->get()->getRowObject();
            ...
            if ($current) {
                ...
                $this->_db->transBegin();
                try {
                    $this->_db->table('tb_matching_pembenahanspj')->where('id', $current->id)->delete();
                } catch (\Throwable $th) {
                    $this->_db->transRollback();
                    $response = new \stdClass;
                    $response->status = 400;
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        unlink(FCPATH . "upload/matching-pembenahanspj/$file.json");
                        unlink(FCPATH . "upload/matching-pembenahanspj/$file");
                    } catch (\Throwable $th) {
                        //throw $th;
                    }
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->message = "Data pembenahanspj berhasil dihapus.";
                    return json_encode($response);
                } else {
                    $this->_db->transRollback();
                    $response = new \stdClass;
                    $response->status = 400;
                    $response->message = "Data pembenahanspj gagal dihapus.";
                    return json_encode($response);
                }
            } else {
                $response = new \stdClass;
