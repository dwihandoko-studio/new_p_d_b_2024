...
namespace App\Controllers\Situgu\Su\Upload\Tamsil;

use App\Controllers\BaseController;
use App\Models\Situgu\Su\Tamsil\Upload\ProsestransferModel;
use Config\Services;
...
class Prosestransfer extends BaseController
{
    ...
    {
        return redirect()->to(base_url('situgu/su/upload/tamsil/prosestransfer/data'));
    }
    ...
    public function data()
    {
        ...
        return view('situgu/su/upload/tamsil/prosestransfer/index', $data);
    }
    ...
    public function upload()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->data = view('situgu/su/upload/tamsil/prosestransfer/upload', $data);
            return json_encode($response);
        }
    }
    ...
    public function uploadSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $dir = FCPATH . "upload/matching-prosestransfer-tamsil";
            $field_db = 'filename';
            $table_db = 'tb_matching_prosestransfer_tamsil';
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                $response->data = view('situgu/su/upload/tamsil/prosestransfer/verifi-upload', $x);
    ...
    public function get_data_json()
    {
        ...
        $datas = json_decode(file_get_contents(FCPATH . "upload/matching-prosestransfer-tamsil/$id.json"), true);
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                if ($status == "table-success") {
                    // try {
                    $this->_db->table('_tb_usulan_detail_tamsil')->where('id', $current->id_usulan)->update(['status_usulan' => 5, 'updated_at' => date('Y-m-d H:i:s'), 'date_matching' => date('Y-m-d H:i:s'), 'admin_matching' => $user->data->id]);
                    if ($this->_db->affectedRows() > 0) {
                        ...
                        if ($ptk) {
                            ...
                            if ($this->_db->affectedRows() > 0) {
                                $this->_db->table('_tb_usulan_detail_tamsil')->where(['id' => $ptk->id])->delete();
                                if ($this->_db->affectedRows() > 0) {
    ...
    public function delete()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('tb_matching_prosestransfer_tamsil')
                ->where('id', $id)
                ->get()->getRowObject();
            ...
            if ($current) {
                ...
                $this->_db->transBegin();
                try {
                    $this->_db->table('tb_matching_prosestransfer_tamsil')->where('id', $current->id)->delete();
                } catch (\Throwable $th) {
                    $this->_db->transRollback();
                    $response = new \stdClass;
                    $response->status = 400;
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        unlink(FCPATH . "upload/matching-prosestransfer-tamsil/$file.json");
                        unlink(FCPATH . "upload/matching-prosestransfer-tamsil/$file");
                    } catch (\Throwable $th) {
                        //throw $th;
                    }
                    $response = new \stdClass;
