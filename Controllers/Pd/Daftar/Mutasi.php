...
class Mutasi extends BaseController
{
    ...
    {
        return redirect()->to(base_url('pd/daftar/mutasi/data'));
    }
    ...
    public function data()
    {
        ...
        if ($cekAvailableRegistered) {
            if ((int)$cekAvailableRegistered->status_pendaftaran !== 3) {
                ...
                return view('pd/daftar/mutasi/index_hasregister', $data);
            }
        }
        ...
        if ($canDaftar->code !== 200) {
            $data['error_tutup'] = $canDaftar->message . " untuk <b>Jalur Mutasi</b>.";
        }
    ...
    public function nextdata()
    {
        ...
        return view('pd/daftar/mutasi/index', $data);
    }
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($email == $user->data->email && $nohp == $user->data->nohp) {
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->url = base_url('pd/daftar/mutasi/nextdata');
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $current = $this->_db->table('_setting_kuota_tb a')
                    ->select("a.sekolah_id, a.mutasi, b.nama, b.npsn, b.status_sekolah_id, b.lintang, b.bujur, b.alamat_jalan, b.desa_kelurahan, b.kecamatan, ROUND(getDistanceKm(b.lintang,b.bujur,'{$userPd->lintang}','{$userPd->bujur}'), 2) AS distance_in_km")
                    // ->select("a.sekolah_id, a.mutasi, b.nama, b.npsn, b.status_sekolah_id, b.lintang, b.bujur, b.alamat_jalan, b.desa_kelurahan, b.kecamatan, DEGREES(ACOS(LEAST(1.0, COS(RADIANS(b.lintang)) * COS(RADIANS({$userPd->lintang})) * COS(RADIANS(b.bujur - {$userPd->bujur})) + SIN(RADIANS(b.lintang)) * SIN(RADIANS({$userPd->lintang}))))) AS distance_in_km")
                    ->join('dapo_sekolah b', 'a.sekolah_id = b.sekolah_id')
                ...
                if (count($current) > 0) {
                    ...
                    $response->data = view('pd/daftar/mutasi/pilihan', $x);
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 200;
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($userPd) {
                    ...
                    $response->message = "Permintaan diizinkan";
                    $response->data = view('pd/daftar/mutasi/validasi', $x);
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 200;
        ...
        if ($this->request->isAJAX()) {
            ...
            $rules = [
                ...
                '_mutasi' => [
                    'rules' => 'required|trim',
                    'errors' => [
                        'required' => 'Mutasi kerja tidak boleh kosong. ',
                    ]
                ],
                '_has_confirmed' => [
                    'rules' => 'required|trim',
            ...
            if (!$this->validate($rules)) {
                ...
                $response->message = $this->validator->getError('_id')
                    ...
                    . $this->validator->getError('_mutasi')
                ...
                $data = [
                    'id' => $uuid,
                    'kode_pendaftaran' => createKodePendaftaran("MUTASI", $oldData->nisn),
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        $riwayatLib->insert($user->data->id, "Mendaftar via Jalur Mutasi, untuk diverifikasi berkas oleh sekolah tujuan.", "Daftar Jalur Mutasi");
                    } catch (\Throwable $th) {
                    }
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->data = $data;
                    $response->url = base_url('pd/daftar/mutasi/data');
    ...
    public function download()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                    ...
                    <tr>
                        <td colspan="3">
                            <ol>
                                ...
                                <li>Jadwal Verifikasi Dokumen Pendaftaran Jalur Mutasi tanggal 26 s/d 28 Juni 2024</li>
                ...
                $filename = 'PENDAFTARAN_PPDB_MUTASI_' . $pendaftaran->nisn_peserta . '.pdf';
