...
class Zonasi extends BaseController
{
    ...
    {
        return redirect()->to(base_url('pd/daftar/zonasi/data'));
    }
    ...
    public function data()
    {
        ...
        if ($cekAvailableRegistered) {
            if ((int)$cekAvailableRegistered->status_pendaftaran !== 3) {
                ...
                return view('pd/daftar/zonasi/index_hasregister', $data);
            }
        }
    ...
    public function nextdata()
    {
        ...
        return view('pd/daftar/zonasi/index', $data);
    }
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($email == $user->data->email && $nohp == $user->data->nohp) {
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->url = base_url('pd/daftar/zonasi/nextdata');
    ...
    public function getDafList()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $current = $this->_db->table('_setting_zonasi_tb c')
                    ->select("a.sekolah_id, a.zonasi, b.nama, b.npsn, b.status_sekolah_id, b.lintang, b.bujur, b.alamat_jalan, b.desa_kelurahan, b.kecamatan, ROUND(getDistanceKm(b.lintang,b.bujur,'{$userPd->lintang}','{$userPd->bujur}'), 2) AS distance_in_km")
                    // ->select("a.sekolah_id, a.zonasi, b.nama, b.npsn, b.status_sekolah_id, b.lintang, b.bujur, b.alamat_jalan, b.desa_kelurahan, b.kecamatan, DEGREES(ACOS(LEAST(1.0, COS(RADIANS(b.lintang)) * COS(RADIANS({$userPd->lintang})) * COS(RADIANS(b.bujur - {$userPd->bujur})) + SIN(RADIANS(b.lintang)) * SIN(RADIANS({$userPd->lintang}))))) AS distance_in_km")
                    ->join('_setting_kuota_tb a', 'a.sekolah_id = c.sekolah_id')
                    ->join('dapo_sekolah b', 'a.sekolah_id = b.sekolah_id')
                    ->where("b.status_sekolah_id = '1' AND c.kelurahan = '{$userPd->kel}' AND c.dusun = '{$userPd->dusun}' $andWhere")
                ...
                if (count($current) > 0) {
                    ...
                    $response->data = view('pd/daftar/zonasi/pilihan', $x);
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 200;
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($userPd) {
                    ...
                    $response->message = "Permintaan diizinkan";
                    $response->data = view('pd/daftar/zonasi/validasi', $x);
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 200;
    ...
    public function exeDaftar()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $data = [
                    'id' => $uuid,
                    'kode_pendaftaran' => createKodePendaftaran("ZONASI", $oldData->nisn),
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        $riwayatLib->insert($user->data->id, "Mendaftar via Jalur Zonasi, untuk diverifikasi berkas oleh sekolah tujuan.", "Daftar Jalur Zonasi");
                    } catch (\Throwable $th) {
                    }
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->data = $data;
                    $response->url = base_url('pd/daftar/zonasi/data');
    ...
    public function download()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                    ...
                    <tr>
                        <td colspan="3">
                            <ol>
                                ...
                                <li>Jadwal Verifikasi Dokumen Pendaftaran Jalur Zonasi tanggal 26 s/d 28 Juni 2024</li>
                ...
                $filename = 'PENDAFTARAN_PPDB_ZONASI_' . $pendaftaran->nisn_peserta . '.pdf';
