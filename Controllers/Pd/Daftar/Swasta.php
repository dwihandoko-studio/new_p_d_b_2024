...
class Swasta extends BaseController
{
    ...
    {
        return redirect()->to(base_url('pd/daftar/swasta/data'));
    }
    ...
    public function data()
    {
        ...
        if ($cekAvailableRegistered) {
            if ((int)$cekAvailableRegistered->status_pendaftaran !== 3) {
                ...
                return view('pd/daftar/swasta/index_hasregister', $data);
            }
        }
        ...
        if ($canDaftar->code !== 200) {
            $data['error_tutup'] = $canDaftar->message . " untuk <b>Sekolah Swasta</b>.";
        }
    ...
    public function nextdata()
    {
        ...
        return view('pd/daftar/swasta/index', $data);
    }
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($email == $user->data->email && $nohp == $user->data->nohp) {
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->url = base_url('pd/daftar/swasta/nextdata');
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($canDaftar->code !== 200) {
                    $response = new \stdClass;
                    $response->status = 400;
                    $response->message = $canDaftar->message . " untuk <b>Sekolah Swasta</b>.";
                    return json_encode($response);
                }
                ...
                $current = $this->_db->table('_setting_kuota_tb a')
                    ->select("a.sekolah_id, (a.afirmasi + a.mutasi + a.zonasi + a.prestasi) as total, b.nama, b.npsn, b.status_sekolah_id, b.lintang, b.bujur, b.alamat_jalan, b.desa_kelurahan, b.kecamatan, ROUND(getDistanceKm(b.lintang,b.bujur,'{$userPd->lintang}','{$userPd->bujur}'), 2) AS distance_in_km")
                    // ->select("a.sekolah_id, (a.afirmasi + a.mutasi + a.zonasi + a.prestasi) as total, b.nama, b.npsn, b.status_sekolah_id, b.lintang, b.bujur, b.alamat_jalan, b.desa_kelurahan, b.kecamatan, DEGREES(ACOS(LEAST(1.0, COS(RADIANS(b.lintang)) * COS(RADIANS({$userPd->lintang})) * COS(RADIANS(b.bujur - {$userPd->bujur})) + SIN(RADIANS(b.lintang)) * SIN(RADIANS({$userPd->lintang}))))) AS distance_in_km")
                    ->join('dapo_sekolah b', 'a.sekolah_id = b.sekolah_id')
                ...
                if (count($current) > 0) {
                    ...
                    $response->data = view('pd/daftar/swasta/pilihan', $x);
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 200;
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($userPd) {
                    ...
                    $response->message = "Permintaan diizinkan";
                    $response->data = view('pd/daftar/swasta/validasi', $x);
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 200;
    ...
    public function exeDaftar()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                ...
                $data = [
                    'id' => $uuid,
                    'kode_pendaftaran' => createKodePendaftaran("SWASTA", $oldData->nisn),
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        $riwayatLib->insert($user->data->id, "Mendaftar ke Sekolah Swasta, untuk diverifikasi berkas oleh sekolah tujuan.", "Daftar Sekolah Swasta");
                    } catch (\Throwable $th) {
                    }
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->data = $data;
                    $response->url = base_url('pd/daftar/swasta/data');
                    $response->message = "Pendaftaran ke Sekolah Swasta berhasil dilakukan.";
                    return json_encode($response);
                } else {
                    $this->_db->transRollback();
                    $response = new \stdClass;
                    $response->status = 400;
                    $response->message = "Pendaftaran ke Sekolah Swasta gagal dilakukan.";
                    return json_encode($response);
                }
            }
        } else {
    ...
    public function download()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($rules)) {
                    ...
                    <tr>
                        <td colspan="3">
                            <ol>
                                ...
                                <li>Jadwal Verifikasi Dokumen Pendaftaran Sekolah Swasta tanggal 24 s/d 28 Juni 2024</li>
                ...
                $filename = 'PENDAFTARAN_PPDB_SWASTA_' . $pendaftaran->nisn_peserta . '.pdf';
