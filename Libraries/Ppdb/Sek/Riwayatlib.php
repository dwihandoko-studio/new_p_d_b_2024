...
namespace App\Libraries\Ppdb\Sek;
...
class Riwayatlib
{
    ...
    public function getAll($userId, $page = "1", $keyword = "")
    {
        ...
        if ($keyword == "") {
            $data['result'] = $this->_db->table('riwayat_system_sekolah')
                ->where('user_id', $userId)
                ->orderBy('created_at', 'desc')
                ->limit($limit_per_page, $start)
                ->get()->getResult();
            $data['countData'] = $this->_db->table('riwayat_system_sekolah')->where('user_id', $userId)
                ->countAllResults();
            $data['page'] = $page;
    ...
    public function insert($userId, $keterangan, $aksi = "merubah data", $icon = "submit")
    {
        ...
        return $this->_db->table('riwayat_system_sekolah')->insert($dataInsert);
    }
}
