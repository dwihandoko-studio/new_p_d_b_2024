                ...
                <div class="table-responsive">
                    <table class="table table-bordered border-primary mb-0 modals-datatables-datanya" id="modals-datatables-datanya">
                        <thead>
                            <tr>
                                <th rowspan="2">#</th>
                                <th colspan="4">DATA PTK LAMA</th>
                                <th colspan="4">DATA BARU UPLOAD</th>
                            </tr>
                            <tr>
                                <th>NAMA</th>
                                <th>NUPTK</th>
        ...
        fetch("./get_data_json?id=<?= $id ?>")
            ...
            .then(data => {
                ...
                for (let i = 0; i < data.data.length; i++) {
                    ...
                    const noRekeningCell = document.createElement("td");
                    const cabangBankCell = document.createElement("td");
                    const namaCellUp = document.createElement("td");
                    const nuptkCellUp = document.createElement("td");
                    const noRekeningCellUp = document.createElement("td");
                    const cabangBankCellUp = document.createElement("td");

                    numberCell.textContent = 1 + i;
                    namaCell.textContent = data.data[i].nama;
                    nuptkCell.textContent = data.data[i].nuptk;
                    noRekeningCell.textContent = data.data[i].no_rekening;
                    cabangBankCell.textContent = data.data[i].cabang_bank;
                    namaCellUp.textContent = data.data[i].nama_up;
                    nuptkCellUp.textContent = data.data[i].nuptk_up;
                    noRekeningCellUp.textContent = data.data[i].no_rekening_up;
                    cabangBankCellUp.textContent = data.data[i].cabang_bank_up;

                    row.appendChild(numberCell);
                    row.appendChild(namaCell);
                    row.appendChild(nuptkCell);
                    row.appendChild(noRekeningCell);
                    row.appendChild(cabangBankCell);
                    row.appendChild(namaCellUp);
                    row.appendChild(nuptkCellUp);
                    row.appendChild(noRekeningCellUp);
                    row.appendChild(cabangBankCellUp);
